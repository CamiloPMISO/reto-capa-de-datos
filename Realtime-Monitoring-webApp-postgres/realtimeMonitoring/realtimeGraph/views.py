...
"""
def get_avg_measure_by_city_json(request, **kwargs):
    data_result = {}
    ...
    for location in locations:
        ...
        avgVal += locationData.aggregate(Avg('value'))['value__avg']

    startFormatted = start.strftime("%d/%m/%Y") if start != None else " "
    endFormatted = end.strftime("%d/%m/%Y") if end != None else " "

    data_result["measure"] = {
        "measure_type" : measureParam,
        "avg_value": avgVal/len(locations)
    }
    data_result["city"] = city
    data_result["population_station_in_city"] = len(locations)
